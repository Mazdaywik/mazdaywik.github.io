...
Здесь запись `[arg]` означает особым образом размеченное данное, которое
суперкомпилятор не может разбирать на части. Для него это — некоторый
неизвестный константный фрагмент. Он его может либо поместить в остаточную
...
2. Выражение `expr` содержит некоторые вызовы функций, зависящие от параметров.
   Будем считать, что `expr` имеет вид `<F1 [arg]>`, а в функции `F1` есть
   ветвление по различным сужениям параметра.
...
![7010-fibint.dot](7010-fibint.png)
...
Суперкомпиляция нам даст следующий [граф](7020-nrev.png).
...
Входной фрагмент `[1]` нигде не бьётся, фрагмент `[(2 (3 (4 NIL)))]` падает
...
Интерпретатор останавливается, возвращая результат `(4 (3 (2 (1 NIL))))`.
